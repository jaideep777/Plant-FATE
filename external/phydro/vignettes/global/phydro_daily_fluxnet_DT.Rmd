...
df_flux = df_flux %>% mutate(time = as.POSIXct(TIMESTAMP_START, tz = "GMT", format="%Y%m%d%H%M"))
```
...
df_flux %>% filter(as.Date(time) >= as.Date("2020-06-01") &
                 as.Date(time) <= as.Date("2020-06-03") ) %>% 
...
df_flux_daily %>% 
  with(plot(GPP_DT_VUT_REF~as.POSIXct(TIMESTAMP, tz = "GMT"), type="l", col="yellow3"))
...
```{r}
test.3day = df_flux %>% filter(as.Date(time) >= as.Date("2020-06-01") &
                 as.Date(time) <= as.Date("2020-06-03") ) 

test.1day = df_flux %>% filter(as.Date(time) == as.Date("2020-06-01"))

test.3day.daily.custom = test.3day %>% group_by(as.Date(time)) %>% do(aggregate_daily_fluxnet(.))

test.3day %>% select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, LAI, FPAR, GPP_DT_VUT_REF) %>% 
  melt("time") %>% 
  mutate(type="hourly") %>% 
  rbind(test.3day.daily.custom %>% ungroup() %>% 
          select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, LAI, FPAR, GPP_DT_VUT_REF) %>% 
          melt("time") %>% 
          mutate(type="daily")
  ) %>% 
...
if (recompute_daily_ts){
  df_daily = df_flux %>% group_by(as.Date(time)) %>% do(aggregate_daily_fluxnet(.))
  df_daily %>% write.csv(file="~/Downloads/flux_data_kit_beta/df_daily_fluxnet.csv", row.names = F)
} else {
  df_daily = read.csv(file="~/Downloads/flux_data_kit_beta/df_daily_fluxnet.csv")
}
...
df_flux %>% filter(as.Date(time) > as.Date("2010-01-01") & 
...
```{r}
recompute_daily_rollmean_ts = F
if (recompute_daily_rollmean_ts){
  df_daily_rollmean = df_daily %>% select(SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, WS_F, FPAR, LAI)
  dt = (df_daily$time %>% as.numeric() %>% diff())/86400 # time differences in days
  ...
  df_daily_rollmean %>% write.csv(file="~/Downloads/flux_data_kit_beta/df_daily_fluxnet_rollmean.csv", row.names = F)
} else {
  df_daily_rollmean = read.csv(file="~/Downloads/flux_data_kit_beta/df_daily_fluxnet_rollmean.csv")
}
...
par_plant_leaf = list(conductivity=0.12e-16, psi50=-1.2, b=1);
...
df_flux_daily_mean = df_flux %>% 
  select(time, SW_IN_F_MDS, TA_F_MDS, VPD_F_MDS, CO2_F_MDS, FPAR, LAI, WS_F, GPP_NT_VUT_REF, GPP_DT_VUT_REF) %>% 
  group_by(as.Date(time)) %>% summarize_all(.funs = mean)

df_flux_daily_mean$LAI[df_flux_daily_mean$LAI > 10] = NA
df_flux_daily_mean$LAI = gapfill(df_flux_daily_mean$LAI)

df_flux_daily_mean$FPAR[df_flux_daily_mean$FPAR > 1] = NA
df_flux_daily_mean$FPAR[df_flux_daily_mean$FPAR < 0] = NA
df_flux_daily_mean$FPAR = gapfill(df_flux_daily_mean$FPAR)

df_flux_daily_mean %>% select(time, LAI, FPAR) %>% 
  melt("time") %>% 
  ggplot(aes(y=value, x=as.POSIXct(time))) + 
  geom_line(col="aquamarine4") + 
  geom_vline(xintercept = 25, col="pink") + 
...
daily_fulldaymean_phydro = df_flux_daily_mean %>% 
  filter(as.Date(time) > as.Date("2011-01-01") & 
         as.Date(time) < as.Date("2021-01-01")) %>% 
  group_by(time) %>% 
  summarize(
...
df_flux_daily_mean %>% 
  filter(as.Date(time) > as.Date("2011-01-01") & 
         as.Date(time) < as.Date("2021-01-01")) %>% 
  select(time, GPP_DT_VUT_REF, SW_IN_F_MDS) %>%
  mutate(time = as.Date(time)) %>% 
  left_join(inst) %>% 
  # filter(a>0) %>% 
  ggplot() + 
...
```{r}
test.6day = df_flux %>% filter(as.Date(time) >= as.Date("2015-06-01") &
                 as.Date(time) <= as.Date("2015-06-12") ) 

acc_physio_day = acc_physio %>% mutate(day = as.Date(time)) %>% select(-time)

df_phydro_subdaily = 
  test.6day %>% mutate(day = as.Date(time)) %>% 
  group_by(time) %>% 
  summarize(
    tc = TA_F_MDS, 
    ppfd = SW_IN_F_MDS*2.04, 
...
```{r}
inst.subdaily = df_phydro_subdaily %>% 
  purrr::pmap_dfr(.f = rphydro_instantaneous_analytical_r, 
                   kphio = 0.087, 
                   psi_soil = 0, 
                   rdark = 0.015, 
  ...
  mutate(time = df_phydro_subdaily$time)
...
test.6day %>%
  left_join(inst.subdaily) %>% 
  # filter(a>0) %>% 
  ggplot() + 
  geom_line(aes(x=as.POSIXct(time), y=GPP_DT_VUT_REF), col="yellow3") + 
