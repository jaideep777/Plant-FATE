...
for(i in 1:N){
  ...
  cat(expt_dir, "\n")
  dat = read.delim(paste0("~/codes/Plant-FATE/",output_dir,"/",expt_dir,"/AmzFACE_Y_PFATE_ELE_HD.txt"))
...
wd = seq(350, 2500, length.out=50)

dat_amb = matrix(nrow=50, ncol=length(wd))
for (i in 1:50){
  cat(i, "\n")
  fitness = wd %>% purrr::map_dbl(fitness_wd, zp=zp_amb, co=co_amb, co2=414, lma=runif(1,0.05,0.25), p50 = runif(1,-4.5,-0.5), hmat=runif(1,5,35))
  dat_amb[i,] = fitness
}
...
for (i in 1:50){
  cat(i, "\n")
  fitness = wd %>% purrr::map_dbl(fitness_wd, zp=zp_amb, co=co_amb, co2=614, lma=runif(1,0.05,0.25), p50 = runif(1,-4.5,-0.5), hmat=runif(1,5,35))
  dat_ecae[i,] = fitness
}
...
for (i in 1:50){
  cat(i, "\n")
  fitness = wd %>% purrr::map_dbl(fitness_wd, zp=zp_ele, co=co_ele, co2=614, lma=runif(1,0.05,0.25), p50 = runif(1,-4.5,-0.5), hmat=runif(1,5,35))
  dat_ecee[i,] = fitness
}
...
dat_amb %>% write.csv("~/codes/Plant-FATE/lho_dat_amb.csv")
...
# boxplot(cbind(dat_ecee_opt-dat_amb_opt, dat_ecae_opt-dat_amb_opt), horizontal=T, col=scales::viridis_pal(begin = 0.3, end = 0.8, direction = -1)(3)[1:2])
# abline(v=0, col="pink")
