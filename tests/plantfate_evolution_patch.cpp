...
public:
	...
	Patch(string solver_method, string ode_method) : S(solver_method, ode_method) {
	}
	...
	void initPatch(io::Initializer &I, int patchNo){
		...
		for (int i=0; i<nspp; ++i){
			...
			spp->fg_dx = 0.01;
			spp->trait_variance = vector<double>(2, 0.01);
		...
		// SolverIO sio;
		sio.S = &S;
		string out_dir = I.get<string>("outDir") + "/" + I.get<string>("exptName") + "/Patch" + to_string(patchNo);
		sio.openStreams(out_dir, I);

	}


	void clearPatch(double t){
...
int main(){
	...
	int nspecies = I.getScalar("nSpecies");
	...
	// Create and initialise patches
	vector<Patch*> pa;
	string solver_method = I.get<string>("solver");
	for (int i=0; i<npatches; i++){
		Patch *patch = new Patch(solver_method, "rk45ck");
		patch->initPatch(I, i);
		pa.push_back(patch);
	}

	// Create a global seed rains pool
	double T_seed_rain_avg = I.getScalar("T_seed_rain_avg");
	vector<MovingAverager> seeds_hist(nspecies);
	for (auto& M : seeds_hist) M.set_interval(T_seed_rain_avg);

	// auto after_step = [&pa, &seeds_hist](double t){
...
// 	double t_clear = 105000;
	...
	for (double t=y0; t <= yf; t=t+delta_T) {
		cout << "stepping " << setprecision(6) << pa[0]->S.current_time << " --> " << t << "\n"; 
			for (Patch* p : pa){
			cout << "Patch " << p->patchID << ": (";
			for (auto spp : p->S.species_vec) cout << spp->xsize() << ", ";
			cout << ")" << endl;
		}
